# 💬 多轮对话使用指南

## 🚀 快速开始

### 1. 训练模型（如果还没有训练过）

```bash
python train.py
```

训练完成后会自动保存模型到 `model.pt`

### 2. 启动交互式对话

```bash
python chat.py
```

然后就可以开始对话了！

---

## 📖 使用示例

### 交互式对话

```bash
$ python chat.py

🤖 正在加载模型...
✅ 成功加载模型: model.pt
✅ 模型加载完成！开始对话吧～

============================================================
🎉 欢迎使用 Mini Transformer 对话系统！
============================================================

💡 使用说明:
  - 输入文本，模型会生成回复
  - 输入 'quit' 或 'exit' 退出
  - 输入 'history' 查看对话历史
  - 输入 'clear' 清空对话历史
  - 输入 'help' 显示帮助信息

============================================================

👤 你: a b c
🤖 模型: c b a

👤 你: b a c
🤖 模型: c a b

👤 你: history
📝 最近 2 轮对话:
--------------------------------------------------
👤 用户: a b c
🤖 模型: c b a
👤 用户: b a c
🤖 模型: c a b
--------------------------------------------------

👤 你: quit
👋 再见！
```

---

## 🎯 功能说明

### 1. 基本对话

直接输入文本，模型会生成回复：

```
👤 你: a b c
🤖 模型: c b a
```

### 2. 查看对话历史

输入 `history` 查看最近的对话：

```
👤 你: history
📝 最近 5 轮对话:
--------------------------------------------------
👤 用户: a b c
🤖 模型: c b a
...
--------------------------------------------------
```

### 3. 清空对话历史

输入 `clear` 清空所有对话历史：

```
👤 你: clear
✅ 对话历史已清空
```

### 4. 退出对话

输入 `quit`、`exit` 或 `q` 退出：

```
👤 你: quit
👋 再见！
```

---

## 🔧 高级用法

### 在代码中使用

#### 方式1：使用 ChatBot 类

```python
from chat import ChatBot

# 创建对话机器人
bot = ChatBot("model.pt")

# 单次对话
response = bot.chat("a b c")
print(response)  # 输出: c b a

# 查看历史
bot.show_history()

# 清空历史
bot.clear_history()
```

#### 方式2：直接使用推理函数

```python
from inference import load_model, predict

# 加载模型
model = load_model("model.pt")

# 预测
result = predict(model, "a b c")
print(result)  # 输出: c b a
```

#### 方式3：批量对话

```python
from chat import batch_chat

# 批量处理多个输入
inputs = ["a b c", "b a c", "c a b"]
batch_chat("model.pt", inputs)
```

---

## 📝 对话历史管理

### 对话历史存储

`ChatBot` 类会自动记录所有对话：

```python
bot = ChatBot("model.pt")

bot.chat("a b c")  # 第1轮
bot.chat("b a c")  # 第2轮

# 查看历史
bot.show_history(10)  # 显示最近10轮
```

### 历史数据结构

```python
# conversation_history 是一个列表
[
    ("用户", "a b c"),      # 用户输入
    ("模型", "c b a"),      # 模型回复
    ("用户", "b a c"),      # 下一轮用户输入
    ("模型", "c a b"),      # 下一轮模型回复
    ...
]
```

---

## ⚙️ 配置选项

### 修改模型路径

在 `config.py` 中修改：

```python
MODEL_SAVE_PATH = "my_model.pt"  # 自定义模型路径
```

### 修改最大生成长度

在 `inference.py` 的 `predict` 函数中：

```python
def predict(model, input_text, max_length=20):  # 默认10，可以改大
    ...
```

---

## 🐛 常见问题

### Q1: 模型文件不存在？

**错误信息：**
```
❌ 模型文件不存在！
   请先运行: python train.py
```

**解决方法：**
先运行 `python train.py` 训练模型

---

### Q2: 模型加载失败？

**可能原因：**
1. 模型文件损坏
2. 模型架构不匹配（修改了模型结构但没重新训练）

**解决方法：**
删除 `model.pt`，重新运行 `python train.py`

---

### Q3: 生成的回复为空？

**可能原因：**
1. 输入文本不在词汇表中
2. 模型训练不充分

**解决方法：**
1. 检查输入是否包含词汇表中的token（a, b, c, 1, 2, 3）
2. 增加训练轮数（在 `config.py` 中设置 `EPOCHS = 300`）

---

### Q4: 如何保存对话历史？

可以扩展 `ChatBot` 类，添加保存功能：

```python
import json

def save_history(self, filepath="history.json"):
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(self.conversation_history, f, ensure_ascii=False, indent=2)

def load_history(self, filepath="history.json"):
    with open(filepath, 'r', encoding='utf-8') as f:
        self.conversation_history = json.load(f)
```

---

## 🎓 进阶技巧

### 1. 自定义生成策略

修改 `inference.py` 中的 `predict` 函数，使用不同的解码策略：

```python
# Greedy decoding (当前使用)
next_token = torch.argmax(logits[0, -1], dim=-1).item()

# 可以改为 Top-k sampling
top_k = 3
top_k_logits, top_k_indices = torch.topk(logits[0, -1], top_k)
next_token = top_k_indices[torch.multinomial(torch.softmax(top_k_logits, dim=-1), 1)].item()
```

### 2. 添加温度控制

```python
def predict_with_temperature(model, input_text, temperature=1.0):
    # temperature > 1: 更随机
    # temperature < 1: 更确定
    logits = logits / temperature
    ...
```

### 3. 流式输出

修改 `predict` 函数，支持实时输出：

```python
def predict_streaming(model, input_text, callback=None):
    # callback 是每次生成token时调用的函数
    for token in generated:
        if callback:
            callback(token)
        ...
```

---

## 📚 相关文件

- `inference.py`: 推理模块（模型预测）
- `chat.py`: 对话模块（交互式界面）
- `train.py`: 训练模块（自动保存模型）
- `config.py`: 配置文件（模型路径等）

---

**现在你可以与训练好的模型进行多轮对话了！** 🎉

