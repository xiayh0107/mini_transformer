...
│   ├── pretrain/          # 预训练模块 (MLM/CLM、数据整理、模型封装)
│   └── transformer/
│       ├── __init__.py
...
并会将权重保存到 `models/pretrain_model.pt`。你可以通过命令行覆盖词表长度、mask 概率、
训练轮数等设置，快速比较“纯训练”与“先预训练后微调”的效果。
...
```bash
python scripts/generate.py --input "a b c"
# Output: 3 1 2
```

支持“压缩”输入（如 `abc`），脚本会自动处理。

### 7. 微调 (Fine-tuning)

针对特定格式（如 `abc - 312`）微调模型：

```bash
python scripts/finetune.py --epochs 50
```

微调后的模型将保存为 `models/finetuned_model.pt`。
